...
public class HomeVideoLogService {
	...
	*/
	...
	/* If the user is not an ADMIN, they can only delete their own projects.
	 * The logged in user name will be compared to the user name of the request.  If they
	 ...
	 */		
	@Transactional (readOnly = false)
	public void deleteProjectById(String userName, Long projectId) {
		AuthUser authUser = new AuthUser();
		...
		if (userName.equals(authUser.getUserName()) || authUser.userIsAdmin) {		
			...
			if (!project.getUser().getUsername().equals(userName)) {
				throw new IllegalStateException("Project with ID=" + projectId +
	...
	*/	
	...
	/* If the user is not an ADMIN, they can only see their own project and locations.
	 * The logged in user name will be compared to the user name of the request.  If they
	 * are different, an exception will be thrown.  The exception is purposely vague
	 * ("No data found") because we don't want the potentially malicious user to even know
	 * if a user with the name they requested even exists.
	 */	
	@Transactional(readOnly = true)
	public List<LocationData> retrieveAllLocationsByUserNameAndProjectId(String userName, Long projectId) {
		AuthUser authUser = new AuthUser();
		if (userName.equals(authUser.getUserName()) || authUser.userIsAdmin) {		
			List<Location> locations = 
					locationDao.findAllByUserNameAndProjectId(userName, projectId);
	...
	/* If the user is not an ADMIN, they can only see their own project and scenes.
	 * The logged in user name will be compared to the user name of the request.  If they
	 * are different, an exception will be thrown.  The exception is purposely vague
	 * ("No data found") because we don't want the potentially malicious user to even know
	 * if a user with the name they requested even exists.
	 */	
	@Transactional(readOnly = true)
	public List<SceneData> retrieveAllScenesByUserNameAndProjectId(String userName, Long projectId) {
		AuthUser authUser = new AuthUser();
		if (userName.equals(authUser.getUserName()) || authUser.userIsAdmin) {		
			List<Scene> scenes = 
					sceneDao.findAllByUserNameAndProjectId(userName, projectId);
			List<SceneData> response = new LinkedList<>();
		
			for (Scene scene : scenes) {
				SceneData sd = new SceneData(scene);
				response.add(sd);
			}				
			return response;
		}
	...
	//* Retrieve all actors for a project
	
	/* If the user is not an ADMIN, they can only see their own project and actors.
	 * The logged in user name will be compared to the user name of the request.  If they
	 * are different, an exception will be thrown.  The exception is purposely vague
	 * ("No data found") because we don't want the potentially malicious user to even know
	 * if a user with the name they requested even exists.
	...
	public List<ActorData> retrieveAllActorsByUserNameAndProjectId
		...
		if (userName.equals(authUser.getUserName()) || authUser.userIsAdmin) {
			List<Actor> actors =
					actorDao.findAllByUserNameAndProjectId(userName, projectId);
			List<ActorData> response = new LinkedList<>();
			
			for (Actor actor : actors) {
				ActorData actd = new ActorData(actor);
				response.add(actd);
			}
			return response;
		}
