...
//* Add a new project
	...
	//* Update a project
		...
		@Tag(name = "04. Project", description = "Project level Operations")
		@Operation(description = "Updates a project for a given user",
				   summary = "Update a project for a user")
		@PutMapping("/user/{userName}/project/{projectId}")
		@ResponseStatus(code = HttpStatus.CREATED)
		public ProjectData updateProject(@PathVariable String userName
										,@PathVariable Long projectId
	            , @io.swagger.v3.oas.annotations.parameters.RequestBody(
	           		 content = @io.swagger.v3.oas.annotations.media.Content(
	           				 examples = {
	           						 @ExampleObject(
	           		value = "{\"projectName\": \"My Wonderful Family\"}"
	           		)  }))
	            						@RequestBody ProjectData projectData) {
			projectData.setProjectId(projectId);
			log.info("Updating project {} for user with name={}", projectData, userName);
			return homeVideoLogService.saveProject(userName, projectData);		
		}
	...
	//* Delete a project
		...
		@Tag(name = "04. Project", description = "Project level Operations")
		@Operation(description = "Deletes a project (must provide owner's user ID)",
				   summary = "Delete a project")		
		@DeleteMapping("/user/{userName}/project/{projectId}")
		public Map<String, String> deleteProjectByID(@PathVariable String userName
				                                   , @PathVariable Long projectId) {
			log.info("Deleting project with ID={}", projectId);
			
			homeVideoLogService.deleteProjectById(userName, projectId);
			
			return Map.of("message", "Deletion of project with ID=" + projectId
					+ " was succesful.");
		}
...
//* Retrieve the list of all locations for a project

	@Tag(name = "11. Location", description = "Location operations")
	@Operation(description = "Gets a list of all locations for a given project " +
            "(must provide owner's user ID)",
			   summary = "Get a list of all locations for a project")
	@GetMapping("/user/{userName}/project/{projectId}/location")
	public List<LocationData> retrieveAllLocationsByUserNameAndProjectId(@PathVariable String userName
			                                                           , @PathVariable Long projectId) {
		log.info("Returning locations for user name={} and project ID={}", userName, projectId);
		return homeVideoLogService.retrieveAllLocationsByUserNameAndProjectId(userName, projectId);
	}
...
//* Delete a file
	...
	//* Retrieve the list of all actors for a project
		
		@Tag(name = "13. Actor", description = "Actor operations")
		@Operation(description = "Retrieves a list of all actors for a given project " +
	            							"(must provide owner's user ID)",
				   summary = "Get a list of all actors for a project")
		@GetMapping("/user/{userName}/project/{projectId}/actor")
		public List<ActorData> retrieveAllActorsByUserNameAndProjectId
											(@PathVariable String userName
										   , @PathVariable Long projectId) {
			log.info("Returning actors for user name={} and project ID={}", userName, projectId);
			return homeVideoLogService.retrieveAllActorsByUserNameAndProjectId(userName, projectId);		
		}
...
//* Retrieve the list of all scenes for a project
	
	@Tag(name = "12. Scene", description = "Scene operations")
	@Operation(description = "Retrieves a list of all scenes for a given project " +
            							"(must provide owner's user ID)",
			   summary = "Get a list of all scenes for a project")
	@GetMapping("/user/{userName}/project/{projectId}/scene")
	public List<SceneData> retrieveAllScenesByUserNameAndProjectId
										(@PathVariable String userName
									   , @PathVariable Long projectId) {
		log.info("Returning scenes for user name={} and project ID={}", userName, projectId);
		return homeVideoLogService.retrieveAllScenesByUserNameAndProjectId(userName, projectId);		
	}
